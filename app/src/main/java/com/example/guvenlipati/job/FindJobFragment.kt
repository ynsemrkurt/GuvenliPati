                        ...
                        databaseReferenceJobs.addListenerForSingleValueEvent(object :
                            ...
                            override fun onDataChange(jobsSnapshot: DataSnapshot) {
                                ...
                                for (dataSnapshot: DataSnapshot in jobsSnapshot.children) {
                                    ...
                                    job?.let {
                                        if (userSnapshot.child(job.petSpecies + "Backer").getValue(Boolean::class.java) == true && job.userID != FirebaseAuth.getInstance().currentUser?.uid.toString() && !startDate.before(currentDate) && job.jobStatus) {
                                            jobList.add(it)
                                        }
                                    }
                                }
