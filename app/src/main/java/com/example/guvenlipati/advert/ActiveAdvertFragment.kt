...
class ActiveAdvertFragment : Fragment() {
                    ...
                    if (offer.offerUser == firebaseUser?.uid && !offer.offerStatus && offer.priceStatus) {
                        ...
                        databaseProcess("jobs", offer.offerJobId, Job::class.java) { job ->
                            ...
                            databaseProcess("pets", job.petID, Pet::class.java) { pet ->
                                petList.add(pet)
                                databaseProcess("users", offer.offerBackerId, User::class.java) { user ->
                                    userList.add(user)
                                    databaseProcess("identifies", offer.offerBackerId, Backer::class.java) { backer ->
    ...
    fun <T> databaseProcess(reference: String, child: String, claSs: Class<T>, onSuccess: (T) -> Unit) {
        FirebaseDatabase.getInstance().getReference(reference).child(child).addListenerForSingleValueEvent(object : ValueEventListener {
